# a.py — GPU-enabled MIDI generator (.mid output)
...
# Direct Metal imports without exception handling
...
try:
    ...
    try:
        if use_gpu:
            ...
            duration_indices = generate_values_gpu(base_seed + 2, num_chords, 'duration_index')  # Use GPU for all
    ...
    if index == 0:
        print(f"Debug - Generation method: {generation_method}")
...
use_gpu = True    # ENABLED GPU
max_workers = 6   # Increased workers for better parallelization
...
if use_gpu and METAL_AVAILABLE:
    ...
    if processor:
        ...
        # Test GPU with a small batch
        try:
            test_values = processor.generate_random_values(12345, 10)
            print(f"🧪 GPU test successful: {test_values[:3]}...")
        except Exception as e:
